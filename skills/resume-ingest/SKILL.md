---
name: resume-ingest
description: "Process raw career material (old resumes, LinkedIn exports, cover letters, past job postings) sitting in a resume-kit workspace's 01_intake/ folder, and merge it into career.yaml, deduping the same job across multiple sources, tagging skills, keeping a source trail for every fact, and asking the user when something is ambiguous or doesn't fit the schema. Use when the user has dropped resume material into 01_intake/ and wants it absorbed into their career database, or says 'ingest my resumes', 'process the intake', 'update my career.yaml from these files'."
---
...
rules literally, especially rule 1 (never fabricate) and rule 3 (ask when
ambiguous).

## Workflow

1. **List `<data_dir>/01_intake/`** (root only, excluding `CONTEXT.md` and
   `output/`). If empty, tell the user there's nothing to ingest and stop.
2. **Read `<data_dir>/02_career-db/career.yaml`** to see what's already
   there. You're merging into this, not starting over.
3. **For each intake file, extract raw content:**
...
9. **Write the updated `career.yaml`** to
   `<data_dir>/02_career-db/career.yaml`, bumping `meta.last_ingested` to
   today's date.
10. **Move processed files** from `<data_dir>/01_intake/` to
    `<data_dir>/01_intake/output/processed/` so a re-run doesn't re-ingest
    the same material.
11. **Write an ingestion report** to
    `<data_dir>/02_career-db/output/<YYYY-MM-DD>-ingest.md`: entries added
    vs. merged into existing ones, skills now tracked, and every question
    you asked with how it was answered.
12. **Report a short summary** to the user, matching the written report,
